...
def decode_huffman_block(b, out):
    ...
    # CHANGED: collect pre-BWT data in a bytearray instead of list of bytes
    buffer = bytearray()
    t = None
    while True:
        decoded -= 1
        ...
        if 0 <= r <= 1:
            ...
            # extend repeated first favourite
            buffer.extend(favourites[0] * repeat)  # bytes * N works
            repeat = 0
    ...
    nt = nearly_there = bwt_reverse(bytes(buffer), pointer)
    i = 0
    # Run-length decoding step
    while i < len(nearly_there):
        if i < len(nearly_there) - 4 and nt[i] == nt[i + 1] == nt[i + 2] == nt[i + 3]:
            # CHANGED: extend instead of append since we add multiple bytes
            out.extend(nearly_there[i:i + 1] * (ord(nearly_there[i + 4:i + 5]) + 4))
            i += 5
        else:
            # CHANGED: extend with a single-byte slice
            out.extend(nearly_there[i:i + 1])
            i += 1
